...
export default function useNestActions() {
  ...
  const [state, update] = useStorageSyncedImmerState(
    {
      nest: nest,
  ...
  const removeItemFromNest = useCallback(
    (itemId: number | string): void => {
      ...
      const collectionsIncludingId = state.collections.filter((collection) =>
        collection.items.includes(itemId.toString()),
      )

      if (collectionsIncludingId.length > 0) {
    ...
    [state.nest, state.collections, update],
  )
